...
export default class Renderer {
  ...
  drawLineClearBursts(effect, visual) {
    if (!effect.bursts || effect.bursts.length === 0 || visual.burstAlpha <= 0) {
      return;
    }
    ...
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';
    effect.bursts.forEach((burst) => {
      const alpha = Math.min(0.72, visual.burstAlpha * burst.strength);
      if (alpha <= 0) {
        return;
      }
      ...
      if (burst.kind === 'row') {
        ...
        const gradient = this.createLinearGradient(boardRect.x, y, boardRect.x + boardRect.width, y);
        gradient.addColorStop(0, 'rgba(110, 214, 255, 0)');
        gradient.addColorStop(0.5, `rgba(255, 224, 92, ${alpha})`);
        gradient.addColorStop(1, 'rgba(110, 214, 255, 0)');
        ctx.fillStyle = gradient;
        ctx.fillRect(boardRect.x, y - height / 2, boardRect.width, height);
        return;
      }
      ...
      if (burst.kind === 'col') {
        ...
        const gradient = this.createLinearGradient(x, boardRect.y, x, boardRect.y + boardRect.height);
        gradient.addColorStop(0, 'rgba(110, 214, 255, 0)');
        gradient.addColorStop(0.5, `rgba(110, 214, 255, ${alpha})`);
        gradient.addColorStop(1, 'rgba(110, 214, 255, 0)');
        ctx.fillStyle = gradient;
        ctx.fillRect(x - width / 2, boardRect.y, width, boardRect.height);
        return;
      }
  ...
  drawLineClearSweeps(effect, visual) {
    if (visual.sweepProgress <= 0 || visual.sweepProgress >= 1) {
      return;
    }

    const { ctx, layout } = this;
    const { boardRect, cellSize } = layout;
    const sweepWidth = Math.max(8, cellSize * 1.8);
    ctx.save();
    ctx.globalAlpha = 0.62 * (1 - Math.abs(visual.sweepProgress - 0.5) * 1.2);

    effect.clearedRows.forEach((row) => {
      const x = boardRect.x + boardRect.width * visual.sweepProgress - sweepWidth / 2;
      const y = boardRect.y + row * cellSize;
      const gradient = this.createLinearGradient(x, y, x + sweepWidth, y);
      gradient.addColorStop(0, 'rgba(110, 214, 255, 0)');
      gradient.addColorStop(0.5, 'rgba(255, 246, 196, 0.72)');
      gradient.addColorStop(1, 'rgba(110, 214, 255, 0)');
      ctx.fillStyle = gradient;
      ctx.fillRect(x, y + 1, sweepWidth, cellSize - 2);
    });

    effect.clearedCols.forEach((col) => {
      const x = boardRect.x + col * cellSize;
      const y = boardRect.y + boardRect.height * visual.sweepProgress - sweepWidth / 2;
      const gradient = this.createLinearGradient(x, y, x, y + sweepWidth);
      gradient.addColorStop(0, 'rgba(110, 214, 255, 0)');
      gradient.addColorStop(0.5, 'rgba(255, 246, 196, 0.72)');
      gradient.addColorStop(1, 'rgba(110, 214, 255, 0)');
      ctx.fillStyle = gradient;
      ctx.fillRect(x + 1, y, cellSize - 2, sweepWidth);
    });
  ...
  drawSettingsPanel(state) {
    ...
    if (state.settings.localMembershipEnabled) {
      rows.splice(9, 0, { key: 'memberBenefit', label: '会员福利', value: state.getMembershipBenefitLabel() });
      rows.splice(10, 0, { key: 'disableMembership', label: '关闭本地会员', value: '' });
    }
  ...
  drawAdminPanel(state) {
    ...
    ctx.fillStyle = TEXT_PRIMARY;
    ctx.font = 'bold 28px sans-serif';
    ctx.fillText('管理员验证', layout.screenWidth / 2, panelY + 42);
    ...
    if (state.adminInput) {
      ctx.fillStyle = TEXT_PRIMARY;
      ctx.fillText(state.adminInput, inputRect.x + 16, inputRect.y + 31);
    } else {
      ctx.fillStyle = 'rgba(185, 210, 255, 0.6)';
      ctx.fillText('请输入管理员码', inputRect.x + 16, inputRect.y + 31);
    }
    ...
    if (state.adminError) {
      ...
      ctx.textAlign = 'left';
      ctx.font = '15px sans-serif';
      ctx.fillText(state.adminError, inputRect.x + 2, inputRect.y + 74);
    }
  ...
  drawMembershipPanel(state) {
    ...
    ctx.fillStyle = TEXT_PRIMARY;
    ctx.font = 'bold 28px sans-serif';
    ctx.fillText('输入会员码', layout.screenWidth / 2, panelY + 42);
    ...
    if (state.membershipInput) {
      ...
      ctx.fillStyle = 'rgba(185, 210, 255, 0.6)';
      ctx.fillText('请输入会员码', inputRect.x + 16, inputRect.y + 31);
    }
