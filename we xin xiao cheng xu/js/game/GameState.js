...
export default class GameState {
  ...
  async trySilentLogin() {
    if (!this.authClient || this.isAuthenticating) {
    ...
    try {
      ...
      this.setLoginError(result && result.error ? result.error : '');
      this.applyAuthState(result);
      return this.authState;
    } catch (error) {
      this.setLoginError((error && (error.safeMessage || error.message)) || 'login failed');
  ...
  startDrag(pieceIndex, touchX, touchY, hitArea) {
    ...
    const pieceWidth = (piece.bounds ? piece.bounds.width : 0) * displayCellSize;
    const pieceHeight = (piece.bounds ? piece.bounds.height : 0) * displayCellSize;
    ...
    const visualPosition = modelState
      ? { x: modelState.visualX, y: modelState.visualY }
      : this.getDraggedPieceVisualPosition(
      touchX,
      touchY,
      piece,
      displayCellSize,
  ...
  moveDrag(touchX, touchY) {
    ...
    const placement = modelState
      ? { row: modelState.row, col: modelState.col, canPlace: modelState.canPlace }
      : this.getDraggedPieceBoardPlacement(visualPosition.x, visualPosition.y, piece);

    this.dragState.pointerX = touchX;
    this.dragState.pointerY = touchY;
    this.dragState.visualX = visualPosition.x;
    this.dragState.visualY = visualPosition.y;
  ...
  async submitAdminCode() {
    ...
    if (!normalizeAdminCode(this.adminInput) || !this.authClient) {
      this.adminError = '验证失败';
      return false;
    }

    try {
      const result = await this.authClient.verifyAdmin(this.adminInput);
      if (result && result.adminMode) {
        this.applyAuthState({
          ...this.authState,
          loggedIn: true,
          isAdminAllowed: true
      ...
      // Keep local play available when backend validation fails.
    }
  ...
  verifyAdminCode(value) {
    return !!normalizeAdminCode(value) && !!this.authState.isAdminAllowed;
  }
